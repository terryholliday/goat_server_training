...
const CorrectIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6 text-green-600 flex-shrink-0">
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
    </svg>
);

const IncorrectIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6 text-rose-600 flex-shrink-0">
...
const createInitialState = (questionsSource: MCQ[]) => {
...
export const Quiz: React.FC<QuizProps> = ({ title, accent = "amber", questionsSource, onComplete, passingThreshold = 0.8 }) => {
  ...
  if (showResult) {
    const correctCount = questions.reduce((acc, q, i) => {
      // Find the original question from the source to get the correct answer text
      ...
      // Get the text of the answer the user selected
      const selectedAnswer = answers[i] !== null ? q.options[answers[i]!] : null;
      // Compare the text to see if they are correct
    ...
    return (
      <SectionWrapper title={`${title} — Results`} accent={accent}>
        <div className="bg-white border rounded-lg p-6 shadow-sm space-y-4">
          <div className={`p-4 rounded-lg ${passed ? 'bg-green-50' : 'bg-rose-50'}`}>
            ...
            <p className="text-lg font-bold text-center">{pct}%</p>
    ...
    <SectionWrapper title={`${title} - Question ${index + 1} of ${total}`} accent={accent}>
      <div className="space-y-4">
        ...
        <div className="bg-white border rounded-lg p-6 shadow-sm">
          <p className="text-lg font-semibold text-gray-900 mb-4">{currentQuestion.q}</p>
          <div className="grid gap-3">
            {currentQuestion.options.map((option, i) => (
              <button
                key={i}
                onClick={() => setSelected(i)}
                className={`w-full text-left border rounded-md px-4 py-3 transition-colors text-gray-800 ${
                  selected === i
